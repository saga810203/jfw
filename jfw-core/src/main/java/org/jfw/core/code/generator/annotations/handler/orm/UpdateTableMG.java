...
public class UpdateTableMG extends UpdateMethodGenerator {
    ...
    protected void buildQueryParamter(StringBuilder sb) {
        if (this.psshs.length > 0) {
            ...
            for (int i = 0; i < this.index4Where; ++i) {
                if (this.dynamicSql) {
                    if (this.sqlvalues[i].getType().getFieldClass().isPrimitive()) {
    ...
    private void buildDynamicSQLWithHasFixSqlValue(StringBuilder sb) {
        ...
        sb.append("StringBuilder sql = new StringBuilder();sql.append(\"UPDATE ").append(this.tableName)
                .append(" SET ");
    ...
    private void buildDynamicSQL(StringBuilder sb) {
        if (this.fixSqlValue.size() > 0) {
            ...
            if (this.sqlvalues[0].getType().getFieldClass().isPrimitive()) {
    ...
    protected void buildSQL(StringBuilder sb) {
        if (this.dynamicSql) {
            ...
            sb.append("String sql=\"UPDATE ").append(this.tableName).append(" SET ");
            boolean addCama = false;
            for (Map.Entry<String, String> entry : fixSqlValue.entrySet()) {
                if (addCama)
                    sb.append(",");
    ...
    public void aferInit() {
        ...
        for (int i = 0; i < this.fields.length; ++i) {
            ...
            if (columnName.length() == 0)
                columnName = POUtil.fieldName2ColumnName(this.fields[i].getName());

            if (df.primaryKey()) {
                svwhere.add(new SqlValue(1, this.fields[i].getName(), columnName, df.value()));
                continue;
            }
            ...
            if (null != iv && (iv.trim().length() > 0)) {
                ...
                if (df.value().getFieldClass().isPrimitive()) {
                    svs.addFirst(new SqlValue(1, this.fields[i].getName(), columnName, df.value()));
                } else {
                    svs.addLast(new SqlValue(1, this.fields[i].getName(), columnName, df.value()));
                    allPrimitive = false;
                }
            }
        }
        if (svwhere.size() == 0)
            throw new RuntimeException("not found primary key in table " + this.tableName);
        ...
        Collections.sort(svs, new Comparator<SqlValue>() {
            ...
            public int compare(SqlValue o1, SqlValue o2) {
                if (o1.getType().getFieldClass().isPrimitive()) {
                    if (o2.getType().getFieldClass().isPrimitive()) {
                        return 0;
                    } else {
                        return -1;
                    }
